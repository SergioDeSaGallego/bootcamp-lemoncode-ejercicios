...
container_mongo="mongo-challenge"
container_backend="node-challenge-backend"
...
if ! docker ps -a --format '{{.Names}}' | grep -wq $container_mongo ; then
    docker run -d \
    ...
    docker start $container_mongo
fi

if ! docker ps -a --format '{{.Names}}' | grep -wq $container_backend ; then
    docker run -d \
    ...
    echo "usando container existente"
    docker start $container_backend
fi
